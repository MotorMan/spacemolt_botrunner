...
test('bot starts with full mocks', async () => {
  const mod = await import('../src/routines/miner.ts');
  minerRoutine = mod.minerRoutine;

  const ctx = {
    bot: { exec: vi.fn().mockResolvedValue({ result: { modules: [{ id: 'mining_laser_v' }] } }), state: 'running', username: 'TestBot', system: 'sol', poi: null, docked: true, fuel: 100, maxFuel: 100, cargo: 0, cargoMax: 100, hull: 520, maxHull: 520, shield: 130, maxShield: 130, isCloaked: false, factionStorage: [], inventory: [], credits: 50000, refreshStatus: vi.fn(), refreshCargo: vi.fn(), refreshLocation: vi.fn(), refreshShip: vi.fn(), refreshFactionStorage: vi.fn(), refreshStorage: vi.fn() },
    log: vi.fn(),
