...
class MassDisconnectDetectorImpl implements MassDisconnectDetector {
  ...
  trackSessionLoss(botName: string): void {
    if (this.shutdownInitiated) {
      debugLogForBot(botName, "mass_disconnect", "skip", `${botName} - shutdown already initiated`);
      return;
    }
    ...
    debugLogForBot(botName, "mass_disconnect", "track", `${botName} - ${this.sessionLossEvents.length} events, ${uniqueBots.size} unique bots (threshold: ${MASS_DISCONNECT_THRESHOLD})`);

    // Check if threshold exceeded
    if (uniqueBots.size >= MASS_DISCONNECT_THRESHOLD) {
      this.shutdownInitiated = true;
      const affectedBots = [...uniqueBots];
      debugLogForBot(botName, "mass_disconnect", "TRIGGER", `Shutdown triggered by ${botName}, affected: ${affectedBots.join(", ")}`);
      if (this.onTrigger) {
