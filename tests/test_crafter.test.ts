...
describe("resolveFinalVenue rental gating", () => {
  ...
  // Regression: a facility-only recipe (can't be hand-crafted) whose per-run
  // rental fee exceeds the spending limit must NOT hard-block when the server's
  // quote says the job is affordable (have_credits — includes faction treasury).
  // Previously the workshop fallback errored ("can only be made at a facility")
  // and the whole crafting chain was blocked (external_facility_blocked).
  it("proceeds at a facility-only venue over the limit when the server says it's affordable", async () => {
    const facilityOnlyBot = {
      exec: async (_cmd: string, payload: any) => {
        if (payload.preset === "workshop") {
          return {
            error: {
              message:
                "'Assemble Station Core' can only be made at a facility — it can't be hand-crafted at the Station Workshop. Drop preset=workshop to auto-route to a facility.",
            },
          };
        }
        return {
          result: {
            ...
            have_credits: true,
            have_inputs: true,
            venue: "Station Core Foundry",
            venue_type: "external",
            recipe: "assemble_station_core",
          },
        };
      },
    };
    const v = { preset: "prefer_own", allowRental: true, usedOwnFacility: false, missingFacility: true };
    const result = await resolveFinalVenue(
      ctx, facilityOnlyBot as any, "assemble_station_core", "Assemble Station Core", 2,
      v as any, baseSettings({ allowRentalPurchase: true, rentalSpendingLimit: 201000 }),
    );
    expect(result.blocked).toBe(false);
    expect(result.facilityId).toBeUndefined();
    expect(result.preset).toBe("prefer_own");
    // Faction/treasury-covered job is not counted against the personal rental spend.
    expect(result.rentalFee).toBe(0);
  });

  // A facility-only recipe over the limit that the server says is NOT affordable
  // still blocks, with a clear message (no impossible workshop fallback).
  it("blocks a facility-only venue over the limit when it isn't affordable", async () => {
    const facilityOnlyBot = {
      exec: async (_cmd: string, payload: any) => {
        if (payload.preset === "workshop") {
          return {
            error: {
              message:
                "'Assemble Station Core' can only be made at a facility — it can't be hand-crafted at the Station Workshop.",
            },
          };
        }
        return {
    ...
    expect(result.label).toContain("facility-only");
  });
});